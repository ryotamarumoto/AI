...
def architecture(input_shape=(128, 128, 1), latent_dim=100):
    parameters = dict()
    N_layers = 9
...
def read_data(dataset_name="screw"):
    path_to_train_set = f"Users/ryotamarumoto/Work/corpy/archive/dataset/train/good/"
    path_to_test_set = f"Users/ryotamarumoto/Work/corpy/archive/dataset/test/"

    train_data = dict()
    test_data = dict()

    extracted_size = (256, 256)
    ...
    for path_to_data, data_dict in [(path_to_train_set, train_data), (path_to_test_set, test_data)]:
        for category in tqdm(os.listdir(path_to_data)):
            for img_name in tqdm(os.listdir(path_to_data + str(category))):
                ...
                for x_start in range(0, img.shape[0] - extracted_size[0] + 1, extracted_size[0]):
                    for y_start in range(0, img.shape[1] - extracted_size[1] + 1, extracted_size[1]):

                        x_end = x_start + extracted_size[0]
                        y_end = y_start + extracted_size[1]

                        img_ROI = img[x_start:x_end, y_start:y_end]
                        img_resized = cv2.resize(img_ROI, resized_size)
...
def read_data_with_random_crop(dataset_name="screw", N_train=10 ** 4):
    path_to_train_set = f"/Users/ryotamarumoto/Work/corpy/archive/dataset/train/"
...
def train_model(input_shape=(128, 128, 1), dataset_name="", latent_dim=100, training_loss="ssim", load_model=True,
                random_crop=False, batch_size=8):
    # 1) read data
    if random_crop:
    ...
    autoencoder = architecture(input_shape=input_shape, latent_dim=latent_dim)
    autoencoder.summary()
    ...
    if not os.path.exists(f"/Users/ryotamarumoto/Work/corpy/models/AE/AE4/model_weights/{dataset_name}/"):
        os.mkdir(f"/Users/ryotamarumoto/Work/corpy/models/AE/AE4/model_weights/{dataset_name}/")

    path_to_save_model = f"/Users/ryotamarumoto/Work/corpy/models/AE/AE4/model_weights/{dataset_name}/"
    ...
    if load_model:
        if name in os.listdir(f"/Users/ryotamarumoto/Work/corpy/models/AE/AE4/model_weights/{dataset_name}"):
...
def parse_args():
    ...
    parser.add_argument("--dataset_name", type=str, default="screw")
