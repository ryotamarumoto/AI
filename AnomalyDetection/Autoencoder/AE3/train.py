...
from data_aug import generate_image_list, augment_images, read_img
...
class data_flow(Sequence):
    ...
    def __len__(self):
        # num of batches per 1epoch
        return int(np.ceil(len(self.filenames) / float(self.batch_size)))

    def __getitem__(self, idx):
        # idx : suffix, return : batch
        batch_x = self.filenames[idx * self.batch_size:(idx + 1) * self.batch_size]
        batch_x = np.asarray([read_img(filename, self.grayscale) for filename in batch_x], dtype=object)
        batch_x = batch_x / 255.
...
autoencoder.compile(optimizer=optimizer, loss=loss, metrics=['mae'] if loss == 'mse' else ['mse'], run_eagerly=True)
autoencoder.summary()
earlystopping = EarlyStopping(patience=20)
checkpoint = ModelCheckpoint(os.path.join(cfg.checkpoint_dir, '{epoch:02d}-{val_loss:.5f}.hdf5'), save_best_only=True, period=1, mode='auto', verbose=1, save_weights_only=True)
...
n = len(decoded_imgs)
save_snapshot_dir = cfg.checkpoint_dir + '/snapshot/'
