...
def random_crop(img, new_size):
    ...
    image = img[y:y+new_size, x:x+new_size]
    return img
...
def augment_images(filelist, args):
    for filepath, n in filelist:
        ...
        for i in range(n):
            ...
            if random.random() < args.p_horizontal_flip:
            ...
            img_varied = cv2.resize(img, (args.im_resize, args.im_resize))
            cv2.imwrite(output_filepath, img_varied)


def get_patch(img, new_size, stride):
    h, w = img.shape[:2]
    i, j = new_size, new_size
    patch = []
    while i <= h:
        while j <= w:
            patch.append(img[i - new_size:i, j - new_size:j])
            j += stride
        j = new_size
        i += stride
...
def patch2img(patches, img_size, patch_size, stride):
    img = np.zeros((img_size, img_size, patches.shape[3]+1))
